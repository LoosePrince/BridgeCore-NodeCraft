...
export class PluginManager {
  constructor({ projectRoot, logger, config, serverManager, commandHandler, rconManager, messenger, eventBus, outputProcessor, permissionManager, agentDataStore = null, agentManager = null }) {
    ...
    this.commandGroupUsage = new Map();
  }
  ...
  createPluginContext(meta, pluginDir, pluginState) {
    ...
    const ensureCommandGroups = (pathArray) => {
      if (!Array.isArray(pathArray) || pathArray.length < 2) {
        return;
      }
      for (let i = 1; i < pathArray.length; i++) {
        const groupPath = pathArray.slice(0, i);
        const key = groupPath.map((p) => p.toLowerCase()).join(' ');
        const currentUsage = this.commandGroupUsage.get(key) || 0;
        if (currentUsage === 0) {
          const existing = registry.find(groupPath.join(' '));
          if (!existing) {
            const prefixText = this.commandHandler?.prefix || this.config.commands.prefix || '!';
            registry.register(groupPath, {
              description: '插件命令组',
              autoGenerated: true,
              handler: async () => {
                const children = this.listSubCommandNames(registry, groupPath);
                if (children.length > 0) {
                  this.logger.info(
                    `[命令组] ${prefixText}${groupPath.join(' ')} => ${children.join(', ')}`
                  );
                } else {
                  this.logger.info(`[命令组] ${prefixText}${groupPath.join(' ')} 暂无子命令`);
                }
                return true;
              }
            });
        ...
        this.commandGroupUsage.set(key, currentUsage + 1);
        pluginState.commandGroups.push({ key, path: groupPath });
      }
    };
    ...
    return {
      ...
      registerCommand: (path, definition) => {
        ...
        ensureCommandGroups(pathArray);
        registry.register(pathArray, definition);
        const storedPath = [...pathArray];
  ...
  listSubCommandNames(registry, pathSegments) {
    const segments = Array.isArray(pathSegments) ? pathSegments : [pathSegments];
    const prefixLower = segments.map((p) => p.toLowerCase());
    return registry
      .getAllCommands()
      .filter((cmd) => {
        if (cmd.path.length !== segments.length + 1) {
          return false;
        }
        for (let i = 0; i < segments.length; i++) {
          if (cmd.path[i].toLowerCase() !== prefixLower[i]) {
            return false;
          }
        }
        return true;
  ...
  async unloadPlugin(id) {
    ...
    if (state.commandGroups && state.commandGroups.length > 0) {
      state.commandGroups.forEach(({ key, path }) => {
        const current = (this.commandGroupUsage.get(key) || 1) - 1;
        if (current <= 0) {
          this.commandGroupUsage.delete(key);
          const existing = registry.find(path.join(' '));
          if (existing?.autoGenerated) {
            registry.unregister(path);
          }
        } else {
          this.commandGroupUsage.set(key, current);
        }
      });
    }
