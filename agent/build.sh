...
echo ""
echo "[1/8] 编译工具模块..."
$JAVAC -d build -cp "$CLASSPATH" src/com/bridgecore/agent/utils/*.java
if [ $? -ne 0 ]; then
    echo "[错误] 工具模块编译失败"
    exit 1
fi

echo "[2/8] 编译注入模块 (MappingResolver)..."
...
echo "[5/8] 编译 ChatInterceptorTransformer.java..."
...
echo "[7/8] 打包 bcnc-agent.jar (包含 ASM)..."
cd build
