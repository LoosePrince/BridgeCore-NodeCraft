...
/**
 * 玩家列表拦截器 - 字节码转换器
 * 用于拦截获取玩家列表的方法并注入拦截逻辑
 */
public class PlayerListInterceptorTransformer implements ClassFileTransformer {
    private final InjectionConfig config;
    private final MethodLocator methodLocator;
    private final PlayerListInjectionStrategy injectionStrategy;

    public PlayerListInterceptorTransformer(InjectionConfig config) {
        this.config = config;
        this.methodLocator = new MethodLocator(config.getTargetMethod());
        this.injectionStrategy = new PlayerListInjectionStrategy();
    }
    ...
    public byte[] transform(ClassLoader loader, String className,
        ...
        for (String targetName : config.getTargetClassNames()) {
            ...
            if (className.equals(targetName) || internalName.equals(targetInternal) || 
                ...
                matches = true;
                AgentLogger.debug("[PlayerListTransformer] 匹配类: " + classNameWithDots);
                break;
            }
        ...
        AgentLogger.debug("[PlayerListTransformer] 处理类: " + className + " (服务端: " + config.getServerType().getDisplayName() + ")");

        try {
            ClassReader cr = new ClassReader(classfileBuffer);
            ClassWriter cw = new SafeClassWriter(ClassWriter.COMPUTE_MAXS | ClassWriter.COMPUTE_FRAMES, loader);
            ClassVisitor cv = new PlayerListMethodVisitor(cw, className);
            cr.accept(cv, ClassReader.EXPAND_FRAMES);
            AgentLogger.debug("[PlayerListTransformer] 字节码修改成功: " + className);
            return cw.toByteArray();
        } catch (Exception e) {
            AgentLogger.error("[PlayerListTransformer] 修改失败: " + e.getMessage(), e);
            return classfileBuffer;
        }
    }
     ...
     * 类访问器 - 修改类的方法
     */
    class PlayerListMethodVisitor extends ClassVisitor {
        private final String className;

        public PlayerListMethodVisitor(ClassVisitor cv, String className) {
        ...
        public MethodVisitor visitMethod(int access, String name, String descriptor,
            ...
            if (matches) {
                AgentLogger.debug("[PlayerListTransformer] 匹配到方法: " + name + descriptor);
            }
            ...
            AgentLogger.debug("[PlayerListTransformer] 拦截方法: " + name + descriptor);
            
            // 使用 PlayerListInjectionStrategy 创建方法访问器
